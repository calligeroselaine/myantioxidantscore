      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14">
          <div className="hidden lg:flex items-center w-full min-w-0 relative">
            ...
            >
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="shrink-0 px-1.5 py-2 text-[10px] font-semibold text-gray-700 hover:text-[#0C2A2C] tracking-wide whitespace-nowrap transition-colors duration-150"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
              <Link
                ...
                className="px-4 py-2 bg-[#0F8A7D] hover:bg-[#0A6358] text-white text-[11px] font-bold rounded-full tracking-wide whitespace-nowrap transition-colors duration-200"
              >
          ...
          <div className="lg:hidden flex items-center justify-between w-full">
            <span className="text-xs font-bold text-[#0C2A2C] tracking-widest">MY ANTIOXIDANT SCORE</span>
            <button
              className="p-2 text-gray-600 hover:text-[#0C2A2C]"
        ...
        <nav className="bg-white border-t border-gray-100 px-4 py-3 space-y-0.5">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="block px-3 py-2.5 text-xs font-semibold text-gray-700 hover:text-[#0C2A2C] hover:bg-gray-50 rounded tracking-wide transition-colors"
          ...
          <Link
            href="/partner-with-us#partner-form"
            className="block mt-2 px-3 py-3 bg-[#0F8A7D] hover:bg-[#0A6358] text-white text-center text-xs font-bold rounded-full tracking-wide transition-colors"
            onClick={() => setMenuOpen(false)}
          >
            Book a Discovery Call
          </Link>
